...
export class GameServer {
  ...
  private handleConnection(ws: WebSocket, req: any) {
    ...
    // Match by matchId or auto-match
    if (matchId) {
  ...
  private autoMatch(ws: WebSocket, address: string, stake: string, denom: string) {
    if (this.waitingPlayer && this.waitingPlayer.ws.readyState === WebSocket.OPEN) {
      const opponent = this.waitingPlayer;
      this.waitingPlayer = null;

      const matchId = uuidv4();
      this.createMatch(matchId, opponent.ws, opponent.address, ws, address, stake, denom);
    } else {
  ...
  private joinOrCreateMatch(ws: WebSocket, address: string, matchId: string, stake: string, denom: string) {
  ...
  private createPendingMatch(matchId: string, ws: WebSocket, address: string, stake: string, denom: string) {
  ...
  private createMatch(
    matchId: string,
    wsA: WebSocket, addressA: string,
    wsB: WebSocket, addressB: string,
    stake: string,
    denom: string,
  ) {
    ...
    console.log(`[GameServer] Match started: ${matchId} (${addressA} vs ${addressB})`);
  }
  ...
  private handleRollDice(match: Match, conn: PlayerConn) {
    ...
    this.broadcastToMatch(match, { type: 'DICE_ROLLED', dice: diceList, player: conn.player });
    this.broadcastState(match);
  ...
  private handleResignDouble(match: Match, conn: PlayerConn) {
    ...
    this.broadcastToMatch(match, { type: 'DOUBLE_RESIGNED', loser: conn.player });
  ...
  private async finishGame(match: Match, winner: Player, multiplier: 1 | 2 | 3) {
    this.clearTurnTimer(match);

    const winnerConn = [...match.players.values()].find(p => p.player === winner);
    const winnerAddress = winnerConn?.address || '';

    const winType = multiplier === 3 ? 'backgammon' : multiplier === 2 ? 'gammon' : 'normal';
    ...
    console.log(`[GameServer] Match ${match.id} ended. Winner: ${winner}, multiplier: ${multiplier}, hash: ${gameHash}`);

    // TODO: call SubmitResult on-chain
  }
  ...
  private handleDisconnect(address: string) {
    ...
    match.disconnectTimer = setTimeout(() => {
      const opponent = [...match.players.values()].find(p => p.address !== address);
      if (opponent) {
  ...
  private broadcastState(match: Match) {
    this.broadcastToMatch(match, { type: 'GAME_STATE', state: match.state, matchId: match.id });
  }

  private broadcastToMatch(match: Match, msg: ServerMessage, excludeAddress?: string) {
