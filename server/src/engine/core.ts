...
export function getLegalMoves(state: GameState): Move[] {
  ...
  if (state.bar[player] > 0) {
    ...
    for (const die of new Set(remainingDice)) {
      const targetIdx = player === "A" ? reEnd - die + 1 : reStart + die - 1;
...
export function applyMove(state: GameState, move: Move): GameState {
  ...
  if (move.from === "bar") {
    ...
    dieUsed = Math.abs(
      (move.to === "off" ? (player === "A" ? -1 : 24) : (move.to as number)) -
        fromIdx,
    );
  }
...
export function checkWin(state: GameState): {
  ...
  if (state.borneOff.A === 15) {
    ...
    if (state.borneOff.B === 0) {
      multiplier =
        state.bar.B > 0 || hasCheckersInWinnerHome(state, "A") ? 3 : 2;
    }
  ...
  if (state.borneOff.B === 15) {
    ...
    if (state.borneOff.A === 0) {
      multiplier =
        state.bar.A > 0 || hasCheckersInWinnerHome(state, "B") ? 3 : 2;
    }
