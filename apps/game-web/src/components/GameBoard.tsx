...
const CHECKER_P0 =
  "bg-neutral-800 border-neutral-500 shadow-[0_4px_8px_rgba(0,0,0,0.9),inset_0_2px_4px_rgba(255,255,255,0.1)]";
const CHECKER_P1 =
  "bg-red-900 border-red-500 shadow-[0_4px_8px_rgba(0,0,0,0.9),inset_0_2px_4px_rgba(255,255,255,0.2)]";
...
const Checker = ({
  ...
  <div
    className={`w-[36px] h-[36px] rounded-full border-[1.5px] flex items-center justify-center text-sm font-bold text-white z-10 ${player === 0 ? CHECKER_P0 : CHECKER_P1}`}
  >
    {countText || ""}
  </div>
);
      ...
      {Array.from({ length: maxRender }).map((_, i) => {
        ...
        const lift =
          isSelected && isLast
            ...
            : "";
        const glow =
          isSelected && isLast && !fastMode
            ? "drop-shadow-[0_0_12px_rgba(255,255,255,0.8)] scale-110 z-30"
            : "";

        return (
          <div
            key={i}
            className={`absolute left-1/2 -translate-x-1/2 transition-all duration-200 ${lift} ${glow}`}
            style={style}
          >
            <Checker
              player={player}
              countText={showNumber ? absCount.toString() : undefined}
            />
          </div>
        );
      })}
...
const Point = ({
  ...
  return (
    <div
      className={`relative flex-1 h-full flex flex-col items-center group ${isLegalDest ? "cursor-pointer" : ""}`}
      onClick={onClick}
    >
      <svg
      ...
      {isLegalDest && (
        <div
          className={`absolute inset-0 z-20 pointer-events-none border-2 border-green-500/50 bg-green-500/10 ${fastMode ? "" : "animate-pulse"}`}
        />
      )}

      {isMovable && !isLegalDest && !isSelected && (
        <div
          className={`absolute inset-0 z-20 pointer-events-none border-2 border-purple-500/40 bg-purple-500/10 shadow-[inset_0_0_15px_rgba(168,85,247,0.3)] ${fastMode ? "" : "animate-pulse"}`}
        />
      )}

      <div
        className={`absolute ${isTop ? "top-1" : "bottom-1"} text-[10px] text-white/10 font-mono pointer-events-none`}
      >
        {index + 1}
      </div>

      <div
        className={`absolute w-full h-[85%] ${isTop ? "top-2" : "bottom-2"} pointer-events-none`}
      >
        <CheckerStack
          count={count}
          isTop={isTop}
...
const Cube = ({ value }: { value: number }) => (
  <div className="relative w-12 h-12 shadow-[0_0_20px_rgba(168,85,247,0.4)] rounded-lg flex items-center justify-center bg-black border border-purple-500/50 overflow-hidden z-20">
    <img
      ...
      alt="Cube"
      className="absolute w-[200%] h-[200%] max-w-none object-cover opacity-80 mix-blend-screen"
    />
    <span className="relative z-10 text-xl font-black text-purple-300 drop-shadow-[0_2px_4px_rgba(0,0,0,1)] font-mono">
      {value}
    </span>
  </div>
);
...
export default function GameBoard({
  ...
  return (
    <div
      className={`w-full max-w-4xl aspect-[1.8/1] rounded-sm shadow-2xl relative flex overflow-hidden border-[12px] transition-colors duration-1000 ${gammonPossible && !fastMode ? "border-red-950 shadow-[0_0_40px_rgba(220,38,38,0.4)]" : "border-neutral-950"} bg-black`}
    >
        ...
        <div className="flex-1 flex w-full">
          {[12, 13, 14, 15, 16, 17].map((idx) => (
            <Point
              ...
              isLegalDest={legalDestinations.includes(idx)}
      ...
      <div className="w-16 h-full bg-neutral-950/90 border-x border-neutral-800 flex flex-col items-center relative shadow-2xl z-20 mx-2 shadow-[0_0_30px_rgba(0,0,0,1)]">
        ...
        {/* P1 Bar (Top side) */}
        <div
          className={`w-full flex-1 relative flex flex-col items-center pt-4 transition-colors ${state.turn === 1 && state.bar[1] > 0 ? "cursor-pointer hover:bg-neutral-800/30" : ""} ${movablePoints?.includes("bar") && state.turn === 1 ? `bg-purple-900/20 shadow-[inset_0_0_20px_rgba(168,85,247,0.3)] ${fastMode ? "" : "animate-pulse"}` : ""}`}
          onClick={() =>
            state.turn === 1 && state.bar[1] > 0 && onPointClick("bar")
          }
        >
          {state.cubeOwner === 1 && (
            <div className="mb-4">
              <Cube value={state.cubeValue} />
            </div>
          )}
          {state.bar[1] > 0 && (
            <div className="mb-1 text-[10px] font-bold text-red-400 uppercase tracking-wider">
              Bar: {state.bar[1]}
            </div>
          )}
          <div className="flex flex-col gap-1 pointer-events-none">
            {Array.from({ length: state.bar[1] }).map((_, i) => (
              <div
                key={`bar1-${i}`}
                className={`transition-all duration-200 ${p1BarSelected && i === state.bar[1] - 1 ? "scale-110 translate-y-2 drop-shadow-[0_0_12px_rgba(255,255,255,0.8)] z-30" : ""}`}
              >
                <Checker player={1} />
              </div>
            ))}
        ...
        <div
          ...
          onClick={() =>
            state.turn === 0 && state.bar[0] > 0 && onPointClick("bar")
          }
        >
          {state.cubeOwner === 0 && (
            <div className="mt-4">
          ...
          {state.bar[0] > 0 && (
            <div className="mt-1 text-[10px] font-bold text-green-400 uppercase tracking-wider">
              Bar: {state.bar[0]}
            </div>
          )}
          <div className="flex flex-col gap-1 pointer-events-none">
            {Array.from({ length: state.bar[0] }).map((_, i) => (
              <div
                key={`bar0-${i}`}
                className={`transition-all duration-200 ${p0BarSelected && i === state.bar[0] - 1 ? "scale-110 -translate-y-2 drop-shadow-[0_0_12px_rgba(255,255,255,0.8)] z-30" : ""}`}
              >
                <Checker player={0} />
              </div>
            ))}
      ...
      <div className="w-16 bg-neutral-950 border-l-2 border-neutral-800 flex flex-col shadow-[-10px_0_20px_rgba(0,0,0,0.8)] z-10 relative">
        ...
        >
          {Array.from({ length: state.off[1] }).map((_, i) => (
            <div
              key={`off1-${i}`}
              className="w-full h-2 mb-[3px] bg-red-900 border border-red-500 rounded-sm shadow-[0_2px_4px_rgba(0,0,0,0.8)] pointer-events-none"
            />
          ))}
        </div>
        ...
        >
          {Array.from({ length: state.off[0] }).map((_, i) => (
            <div
              key={`off0-${i}`}
              className="w-full h-2 mt-[3px] bg-neutral-800 border border-neutral-500 rounded-sm shadow-[0_2px_4px_rgba(0,0,0,0.8)] pointer-events-none"
            />
          ))}
        </div>
      </div>
